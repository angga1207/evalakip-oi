...
class EvaluatorAuthMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        // Check if the user is authenticated and has the evaluator role
        if (!auth()->check() || in_array(auth()->user()->role_id, [1, 3]) === false) {
        ...
        // Proceed with the request if authenticated and has the role
        return $next($request);
    }
}
